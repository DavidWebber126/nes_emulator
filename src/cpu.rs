...
impl<'a> CPU<'a> {
        ...
        F: FnMut(&mut CPU),
    {
        loop {
            ...
            //println!("PC: {:04X} SP: {:04X} OPCODE {:02X} REG_A {:02X} STATUS {:08b}", self.program_counter, self.stack_pointer, opcode.opcode,self.register_a, self.status);
            self.program_counter += 1;
            // println!("next BYTE: {:02X}", self.mem_read(self.program_counter));
            // println!("HI BYTE: {:02X}", self.mem_read(self.program_counter + 1));
...
impl Trace {
    fn new(cpu: &mut CPU) -> Trace {
        ...
        let opcode = opcodes.get(&cpu.mem_read(cpu.program_counter)).unwrap();
        let mut opcodes: Vec<u8> = Vec::new();
        for i in 0..(opcode.bytes) {
            opcodes.push(cpu.mem_read(cpu.program_counter + i));
        }
        ...
        let assembly_text = match opcode.addressing_mode {
            ...
            AddressingMode::Indirect => {
                ...
                let hi = cpu.mem_read(ptr_lo.wrapping_add(1) as u16 | ptr_hi);
                let addr = u16::from_le_bytes([lo, hi]);
